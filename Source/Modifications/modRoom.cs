    ...
    internal class modRoom : IGameModification
    {
        ...
        private static void CheckCheckpoint(Level level, string room)
        {
            ...
            for (int i = 0; i < checkpoints.Length; i++)
            {
                ...
                {
                    long locationID = ArchipelagoMapper.getCheckpointLocationID(SID, mode, i);
                    CelesteArchipelagoModule.SaveData.LocationsChecked.Add(locationID);
                    CelesteArchipelagoModule.Log($"Checkpoint {room} (index {i}) reached in {SID} {mode}, mapping to location id {locationID:X}");
                    break;
                }
            }
        }
